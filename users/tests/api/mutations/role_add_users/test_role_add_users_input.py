...
from users.api.mutations.role_add_users.role_add_users_errors import RoleDoesNotExistError, UserDoesNotExistError, \
...
class TestRoleAddUsersInput(TransactionTestCase):
    ...
    async def test_validate_and_get_errors_with_empty_list_of_user_ids(self) -> None:
        ...
        input_with_empty_list_of_user_ids = RoleAddUsersInput(
            role_id=self.valid_input_values["role_id"],
            users_ids=[]
        )
    ...
    async def test_validate_and_get_errors_no_errors(self) -> None:
        role_add_perm_input = RoleAddUsersInput(**self.valid_input_values)
