...
from users.api.mutations.role_remove_permission.role_remove_permission_errors import RoleDoesNotExistError, \
...
class TestRoleRemovePermissionInput(TransactionTestCase):
    ...
    async def test_validate_and_get_errors_perm_empty(self) -> None:
        input_empty_perm = RoleRemovePermissionInput(
            role_id=self.default_values["role_id"],
            permissions_ids=[]
        )
    ...
    async def test_validate_and_get_errors_no_errors(self) -> None:
        role_add_perm_input = RoleRemovePermissionInput(**self.default_values)
