...
class TestRoleRemoveUsersResolver(TransactionTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.mutation_query = """
            mutation RoleRemoveUsers($input: RoleRemoveUsersInput!) {
              roleRemoveUsers(input: $input) {
                node {
                  id
                }
                userErrors {
    ...
    async def test_role_remove_users_resolver_response(self) -> None:
        role_remove_input = RoleRemoveUsersInput(**self.input)
        result: RoleRemoveUsersPayload = cast(RoleRemoveUsersPayload,
                                              await role_remove_users_resolver(input=role_remove_input))

        # Test if resolver is returning the correct payload
        self.assertIsInstance(result, RoleRemoveUsersPayload)
        ...
        if node:
            ...
            # Test returning the two users removed
            users: Connection[UserType] = await node.users()
            returned_user_ids: List[ID] = [user.node.id for user in users.edges]

            # Test if users have been removed
            self.assertEqual(0, len(returned_user_ids))

    async def test_role_remove_users_resolver_side_effect(self) -> None:
        # Building input
        role_remove_users_input = RoleRemoveUsersInput(**self.input)

        # Removing users to role
        await role_remove_users_resolver(input=role_remove_users_input)
    ...
    async def test_role_remove_users_resolver_permission_denied(self) -> None:
        # create a user without the required `remove_role_to_user` permission
        ...
        if execution_result.data:
            ...
            user_errors: List[dict] = execution_result.data["roleRemoveUsers"]["userErrors"]
            # check that the first error has the field "permission"
            self.assertEqual(user_errors[0]["field"], "permission")

    async def test_role_remove_users_resolver_permission_accepted(self) -> None:
        # Create a user with the required permission to remove users to a role
        user: User = await create_user_with_permission("User", "Password", "remove_role_to_user")
        request = await get_async_request_with_user_and_session(user=user)
